...
export const ProjectListSection = (): JSX.Element => {
  // Project data for easy maintenance and reusability
  const projectData = {
    image: "https://c.animaapp.com/mdhe93boXq2jgO/img/frame-95-4.png",
  ...
  return (
    <div className="flex flex-col w-full max-w-[203px] items-start gap-2.5">
      <Card className="w-full overflow-hidden border-none">
        ...
        <CardContent className="p-0 pt-2.5">
          <div className="flex items-center gap-3 w-full">
            ...
            <h3 className="font-medium text-white text-lg tracking-[-0.48px] leading-7 whitespace-nowrap font-['Satoshi-Medium',Helvetica]">
              {projectData.title}
            </h3>
          </div>
          ...
          <div className="flex items-center gap-1.5 mt-2.5">
            {projectData.categories.map((category, index) => (
              <Badge
                key={index}
                className="h-[30.84px] px-[14.8px] py-0 rounded-[140.61px] text-[10px] font-normal tracking-[-0.20px] leading-[15.8px] text-white font-['Satoshi-Regular',Helvetica] backdrop-blur-[3.65px] backdrop-brightness-[100%] bg-[radial-gradient(50%_50%_at_13%_4%,rgba(18,20,38,0.6)_0%,rgba(0,0,0,0.6)_100%)] before:content-[''] before:absolute before:inset-0 before:p-[0.62px] before:rounded-[140.61px] before:bg-[linear-gradient(180deg,rgba(255,255,255,0.5)_0%,rgba(153,153,153,0.07)_100%)] before:[-webkit-mask:linear-gradient(#fff_0_0)_content-box,linear-gradient(#fff_0_0)] before:[-webkit-mask-composite:xor] before:[mask-composite:exclude] before:z-[1] before:pointer-events-none"
              >
                {category}
              </Badge>
            ))}
